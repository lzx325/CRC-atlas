...
class GroupedIsoformV2(object):
    def __init__(
        ...
        self.illumina_sample_info=IlluminaSampleInfo(**illumina_smaple_info_args)
    ...
    def __setup_gene_id_mapping(self):
        self.gene_name_by_id=pd.read_csv("gene_id_to_name.csv",header=None,index_col=0)[1]
    ...
    def parse_query_transcript_isoform(
        ...
        def get_supporting_molecule_count_by_cell_type(barcode_umi_pair):
            ...
            for barcode,molecule_id in barcode_umi_pair:
                ...
                if barcode in self.cell_barcodes_illumina_intersect:
                    cell_type=self.illumina_sample_info.illumina_barcodes_indexed_table_pacbio_intersect.loc[barcode,self.illumina_sample_info.anndata_field_names["cluster_midway"]]
        ...
        def map_cell_type(cb,with_sample_name=False):
            if cb in self.cell_barcodes_illumina_intersect:
                cell_type=self.illumina_sample_info.illumina_barcodes_indexed_table_pacbio_intersect.loc[cb,self.illumina_sample_info.anndata_field_names["cluster_midway"]]
...
class IlluminaSampleInfo(object):
    def __init__(
        self,
        anndata_fp,
        sample_name_mapping_fp,
        aggr_fp,
        anndata_field_names={
            "cluster_midway":"cluster_midway",
            "cluster_full":"cluster_full",
            "condition":"condition",
        },
        aggr_field_names={
            "illumina_sample_name":"illumina_sample_name",
            "condition":"condition"
        }
    ):
        ...
        aggr_table = aggr_table[[aggr_field_names["illumina_sample_name"],aggr_field_names["condition"]]]
        aggr_table.columns=["illumina_sample_name","condition"]
        ...
        # set up field names
        self.anndata_field_names=anndata_field_names
    ...
    def get_cell_type_sets(self):
        cluster_midway_field = self.anndata_field_names["cluster_midway"]
        cluster_full_field = self.anndata_field_names["cluster_full"]
        condition_field = self.anndata_field_names["condition"]

        cell_barcodes_classified=self.illumina_barcodes_table.groupby(cluster_midway_field)["illumina_indexed_barcode"].unique()
        ...
        cell_barcodes_classified=self.illumina_barcodes_table.groupby(cluster_full_field)["illumina_indexed_barcode"].unique()
        ...
        cell_barcodes_classified=self.illumina_barcodes_table.groupby(condition_field)["illumina_indexed_barcode"].unique()
    ...
    def get_pacbio_intersection(self,pb_barcodes):
        ...
        self.illumina_barcodes_cluster_midway_sn_map_pacbio_intersect=dict(
            zip(
                ...
                zip(self.illumina_barcodes_table_pacbio_intersect["PB_sample_name"],self.illumina_barcodes_table_pacbio_intersect[cluster_midway_field])
            )
        )
        ...
        self.illumina_barcodes_cluster_full_sn_map_pacbio_intersect=dict(
            zip(
                self.illumina_barcodes_table_pacbio_intersect["illumina_indexed_barcode"],
                zip(self.illumina_barcodes_table_pacbio_intersect["PB_sample_name"],self.illumina_barcodes_table_pacbio_intersect[cluster_full_field])
            )
        )
        return filter_stat
    ...
    def get_per_gene_stats2(self,gene_symbol):
        ...
        for cell_type in chain(self.cluster_midway_sets.keys(),["ALL"]):
            if cell_type=="ALL":
                ...
                subset_index=self.anndata.obs.index[self.anndata.obs[cluster_midway_field]==cell_type]
    ...
    def get_all_per_gene_stats(self,log1p=True):
        ...
        for cell_type in cell_types_list:
            if cell_type=="ALL":
                ...
                subset_index_str=self.anndata.obs.index[self.anndata.obs[cluster_midway_field]==cell_type]
...
class PacBioIlluminaCombinedInfo(object):
    ...
    def load_illumina_info(self):
        illumina_cr_aggr_run_root="cr_aggr-Colon_PS017-PS033"
